...
def _build_parser():
    ...
    parser.add_argument('--callback', type=str, action='append', dest='callbacks', required=True,
    ...
    parser.add_argument('--loss', type=str, required=True,
                        help='number of epochs (must be greater than 0)')
    parser.add_argument('--metric', type=str, action='append', dest='metrics', required=True,
                        help='Collection of metrics to monitor, e.g., accuracy, f1')
    parser.add_argument('--metric_emit_freq', type=str, required=True,
                        help='Frequency of metric emission, e.g., '
                             '`lambda epochs: epochs % 10 == 0`, defaults to every epoch')
    parser.add_argument('--optimizer', type=str, action='append', dest='optimizers', required=True,
    ...
    parser.add_argument('--writer', type=Enum('WriterEnum', (('STDOUT', 'stdout'),
                                                             ('STDERR', 'stderr'),
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        trainer.train(callbacks=args.callbacks,
                      epochs=args.epochs,
                      loss=args.loss,
